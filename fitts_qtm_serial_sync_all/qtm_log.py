...
ready_flag = Path(__file__).parent / "session_ready.flag"

# === Wait for folder path and flag ===
print("⏳ Waiting for participant folder info...")

for _ in range(60):  # wait up to 60 seconds
    if session_file.exists() and ready_flag.exists():
        break
    time.sleep(1)
else:
    print("❌ Session path or flag not found in time.")
    sys.exit(1)
...
print(f"📂 Logging QTM to: {log_dir}")
