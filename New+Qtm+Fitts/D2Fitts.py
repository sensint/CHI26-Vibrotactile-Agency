# This is a version of your Fitts' Law experiment code adapted for Difficulty 2:
# W = 80 px, D = 160 px, and uses the proper rectangle bounds in mm
...
SERIAL_PORT = 'COM7'
W_VALUES = [80]  # Width for difficulty 2
D_VALUES = [160]  # Distance for difficulty 2
TOTAL_TRIALS = 20
...
trial_count = {}
difficulty = 1  # Since we only run difficulty 2, we still use index 0
clicks = 0
target_side = 1
previous_rects = []
data = []
...
def handle_serial_click(event=None):
    ...
    throughput = 2 / (mt / 1000) if mt > 0 else 0
...
def save_data_and_finish():
    ...
    filename = f"{participant_name}_{participant_id}_{vibro_label}_ID2_trial{trial_count[participant_id][difficulty]}.xlsx"
