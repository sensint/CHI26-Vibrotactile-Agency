# Fitts' Law Experiment Code for All Difficulties (1-5) with dynamic selection
...
SERIAL_PORT = 'COM7'
W_VALUES = [100, 80, 60, 60, 50]  # widths in px for each difficulty (1-5)
D_VALUES = [100, 160, 240, 480, 800]  # distances in px for each difficulty (1-5)
TOTAL_TRIALS = 20
...
def is_inside_rectangle(localx):
    if difficulty == 1:
        return (-24.33 <= localx <= -8.11) or (8.11 <= localx <= 24.33)
    elif difficulty == 2:
        return (-36.04 <= localx <= -21.63) or (21.63 <= localx <= 36.04)
    elif difficulty == 3:
        return (-49.06 <= localx <= -36.93) or (36.93 <= localx <= 49.06)
    elif difficulty == 4:
        return (-91.90 <= localx <= -81.09) or (81.09 <= localx <= 91.90)
    elif difficulty == 5:
        return (-148.67 <= localx <= -139.65) or (139.65 <= localx <= 148.67)

    return False

def update_fitts_file_with_localx_and_errors(fitts_path, clicked_path):
    try:
...
def save_data_and_finish():
    ...
    ws = wb.active
    ws.title = "Results"
    headers = ['MT', 'speed', 'throughput', 'error', 'LocalX', 'Difficulty']
    ws.append(headers)
    for row in data:
        ws.append([row.get('MT', ''), row.get('speed', ''), row.get('throughput', ''), row.get('error', ''), "", difficulty])
    ...
    avg_error = 0
    if clicked_logs:
        avg_error = update_fitts_file_with_localx_and_errors(filepath, str(clicked_logs[0]))
...
def begin_trial():
    global clicks, data, previous_rects, target_sides
    clicks = 0
    data.clear()
    target_sides.clear()
...
def start_experiment():
    global participant_name, participant_id, vibro_feedback, participant_folder, difficulty
