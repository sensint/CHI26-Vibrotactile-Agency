# Combined QTM + Fitts' Law Experiment with Forced Second Screen Placement
...
if len(monitors) < 2:
    print("⚠️ Only one monitor detected. Defaulting to primary screen.")
    selected_monitor = monitors[0]
else:
    selected_monitor = monitors[1]  # Use second screen

# Hide root and create experiment window
root = tk.Tk()
root.withdraw()
experiment_window = tk.Toplevel()
experiment_window.title("Fitts' Law Experiment")

# Set window geometry and move it to second screen using ctypes (reliable on Windows)
...
# Use ctypes to move window reliably
...
def handle_qtm_data(packet):
    global latest_qtm, screen_corners, mm_to_px_x, mm_to_px_y
    try:
        ...
        if len(latest_qtm) >= 8:
            ...
            print(f"📀 QTM screen size: {screen_width_mm:.2f} mm x {screen_height_mm:.2f} mm")

            mm_to_px_x = CANVAS_WIDTH / screen_width_mm
            mm_to_px_y = CANVAS_HEIGHT / screen_height_mm
            print(f"🖥️ Canvas resolution mapped to: {CANVAS_WIDTH}px x {CANVAS_HEIGHT}px")
